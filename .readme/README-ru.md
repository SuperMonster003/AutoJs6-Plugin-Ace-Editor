...
* `Улучшение` Индексы автодополнения редактора `autojs6_indices.js` и сводное объявление `lib.autojs6.d.ts` заново сгенерированы хостовым `tools/ace-completion` из встроенных объявлений `4.19.0`: добавлены автодополнение и подсказки сигнатур для модуля `epub`; все четыре проверочных скрипта и тест автодополнения проходят
* `Улучшение` Встроенные объявления AutoJs6 `4.19.0` дополнены выделениями и заметками плагина Readium EPUB Reader 1.1.0 (версия 2 контракта EPUB): `epub.annotations` / `annotationsAsync`, `Book.annotations` / `annotationsAsync`, `Internal.Epub.Annotation` / `AnnotationStyle` и событие `highlight` у `ReaderSession` (`HighlightEvent`); индексы автодополнения `autojs6_indices.js` и сводное объявление `lib.autojs6.d.ts` заново сгенерированы хостовым `tools/ace-completion`, все четыре скрипта проверки и тест автодополнения пройдены

# v1.11.0

###### 2026/09/20

* `Улучшение` Встроенные объявления AutoJs6 обновлены до `4.18.0`: добавлены глобальные объекты `mail` / `$mail` и пространство имён `Internal.Mail` плагина Angus Mail (методы клиента и переадресованные методы в синхронной и `Async` формах, события наблюдения, объекты письма / вложения / адреса, параметры учётной записи и предустановки провайдеров, условия поиска, документы результатов и коды ошибок), поэтому автодополнение и проверка типов в редакторе теперь охватывают почтовый API
* `Улучшение` Индексы автодополнения редактора `autojs6_indices.js` и сводное объявление `lib.autojs6.d.ts` заново сгенерированы хостовым `tools/ace-completion` из встроенных объявлений `4.18.0`: добавлены автодополнение и подсказки сигнатур для модулей `mail`, `ai`, `tts`, `flow`, `pangu`, `settings`, `yolo`, `powerManager` и `workManager`, индекс объявлений `index.d.ts` и `BUNDLED_DECLARATIONS.md` возвращены к форме, которую создает скрипт импорта; все четыре проверочных скрипта и тест автодополнения проходят
