...
* `優化` 同步 AutoJs6 4.15.0 宣告中的 images.matchTemplate scales 選項, 模板匹配項幾何屬性及 MatchingResult 輔助方法, 更新資源與相依性宣告並重新產生 LSP 分組
...
* `新增` 內置 AutoJs6 `4.4.0` PNG 量化結果聲明及重新生成的主應用 LSP 分組: `images.quantize` 傳回編碼位元組, 大小, 實際品質與量化誤差; 明確品質下限無法滿足時公開類型化 `QualityTooLowException`

# v1.1.15

###### 2026/08/30

* `新增` 內置 AutoJs6 `4.3.0` PNG 量化選項聲明及重新生成的主應用/資源 LSP 分組: `Images.PngQuantizationOptions` 涵蓋調色板大小, 速度, 品質範圍, 抖動與 posterize, 並保留數字 `quality` 相容性
...
* `新增` `ai.ask`/`ai.chat`/`ai.stream` 同持久 `ai.session` 的明確 backend profile 類型聲明, 覆蓋 `cpu`/`gpu`/`npu`, `ai.catalog` 裝置可用性, 穩定不可用原因及禁止 CPU 回退語義
...
* `新增` 完善 AutoJs6 本機 AI 插件類型聲明, 涵蓋多角色訊息歷史, 官方與第三方選擇器, 生成參數, 精確用量及串流負載, 以及 `ai.catalog` 模型探索
...
* `新增` 同步 AutoJs6 AI 插件的 `Ask`, `Chat` 和 `Stream` 類型聲明, 包括官方/第三方插件選擇, 本地生成參數, 路由響應和串流事件類型
