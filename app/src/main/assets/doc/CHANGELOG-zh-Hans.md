...
* `优化` 内置 AutoJs6 声明升级至 4.14.0, 补充图像像素统计, 均色, 像素读取及颜色距离, 反色, 混合和对比度接口
...
* `修复` Lua 工作区路径, TypeScript 依赖快照和语言服务器测试终止操作的 Android 7 兼容性
...
* `新增` 内置 AutoJs6 声明 4.8.1, 补充控制台输入, 时间格式, 颜色及布局选项, JSX 支持 console/globalconsole 元素 (来源: AutoJs6 6.8.0/5279)

# v1.1.22

###### 2026/09/07

* `新增` 内置 AutoJs6 声明 4.8.0, 支持 Flow 链式自动化, 事件等待, 选择器及诊断接口的补全 (来源: AutoJs6 6.8.0/5278)
...
* `新增` 可插拔八能力语义 Provider, 通用 JSON-RPC/LSP 核心及 WebWorker/设备内 stdio 双传输; TypeScript 已零回归迁移, provider 故障时自动降级至 P2, 四门新语言语义开关默认关闭
...
* `新增` 内置 LuaLS 3.18.2 离线补全, 悬浮信息, 签名, 诊断及定义跳转, 支持 arm64-v8a/armeabi-v7a/x86_64 (不可用时退回静态补全)
* `新增` 基于 ECJ 3.26.0 及 Android API 36 存根的 Java 单文件诊断, 支持语法及未解析符号标注 (补全仍使用静态索引)
* `新增` Kotlin 离线补全, 支持 Kotlin 2.2.21 实例接口, 当前文件类型提示及安全调用, 复用 Java/Android 索引 (不内置 Kotlin 编译器)
...
* `新增` 通过 F2 与移动端 "重命名" 触发的项目范围 TypeScript 符号重命名: Ace 从精确项目快照生成有界跨文件编辑并请求 AutoJs6 contract 5 授权, 预览, 冲突检查, 原子发布, 回滚及全部磁盘写入均完全由宿主负责
* `新增` TypeScript 自动导入与拼写快速修复, 可通过 Ctrl/Command+. 或移动端 "快速修复" 触发: Ace 仅接受当前缓冲区内的有序编辑, 请求 AutoJs6 6.8.0 (5276) 进行 contract 4 授权, 并将批准结果作为一次可撤销修改应用
* `新增` TypeScript 跨文件智能已覆盖项目源码和冻结依赖声明中的补全, hover, 签名帮助及定义跳转; F12, Ctrl/Command+点击和移动端 "转到定义" 会发送 contract 3 目标, 由 AutoJs6 6.8.0 (5276) 在打开和定位前独立复验
* `新增` 宿主提供的 TypeScript 项目诊断: Ace 会校验并加载完整的有界源码快照, 解析项目文件间的 import, 并按运行前编译的相同规则标出缺失模块; 需要 AutoJs6 6.8.0 (5276) 或更高版本
* `新增` TypeScript 项目类型层会在继续编辑前检测原生 addon 信号和安装生命周期 hook, 发布与宿主及编译器一致的稳定依赖边界错误和纯 JavaScript/WASM 替代指引
...
* `新增` 项目依赖类型解析, 支持 types/typings, TypeScript 6 typesVersions, 嵌套声明及已安装的 @types, 与编译插件保持一致
* `新增` 内置 AutoJs6 4.2.0 的 R8 类型声明, 支持 loadJarWithR8 产物导出及 retraceR8Stack 调用栈还原
...
* `新增` TypeScript 诊断与编译插件的 6.0.3 revision-2 配置同步, 支持 Rhino/Node 项目及 .mts/.cts 文件 (保留静态回退)
...
* `新增` ai.session 持久本机会话类型声明, 覆盖固定会话选项, 每轮仅接收新提示词的 ask/chat/stream 方法, 生命周期状态及显式关闭语义
...
* `修复` 可选声明组的静态成员补全, 包括 App.CHROME 和大型 `R.string.text_*` 资源集; Ace 截断候选列表会随前缀变化进行防抖刷新
* `修复` 普通大文档被误判为超长行并切换到纯文本模式, 导致 JavaScript 高亮, 补全和语义服务失效的问题; 真正的超长单行仍会启用安全模式
* `修复` Ace 签名/参数提示气泡始终使用亮色背景的问题, 会随编辑器主题动态应用背景色和前景色; 切换主题时也会单独刷新已存在的自动补全候选菜单
* `修复` 系统文本选择 ActionMode 无法跟随 Ace 配色的问题, 现改用编辑器自有且可感知调色板的选择工具栏, 使选择操作, 按压状态和更多面板在各 Android 版本中均跟随当前 Ace 配色

# v1.1.0

###### 2026/07/27

* `新增` AutoJs6 声明分组选择, core 默认启用, android/libraries/resources/main-app 可按需开启并自动选中依赖分组 (保留完整声明)
* `新增` 提供 `:app:generateAutoJs6LspDeclarations` Gradle 任务, 用于校验声明并生成五个 LSP 分组及其 manifest; 常规资产合并会自动调用该任务, 外部脚本也可直接调用
* `依赖` 升级内置 TypeScript 语言服务及标准库声明版本 4.2.4 -> 6.0.3
